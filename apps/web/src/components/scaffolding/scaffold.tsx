export function Scaffold(props: {
    ...
    return (
        <div className='flex h-full w-full flex-col'>
            {props.appbar && appbarFit === 'above-all' && <header className='w-full shrink-0'>{props.appbar}</header>}
            <div className='relative flex h-full min-h-0 w-full shrink'>
                ...
                <div className='relative flex max-h-[100%] min-h-0 shrink flex-grow flex-col'>
                    {appbarChildren && <header className='z-10 w-full shrink-0'>{props.appbar}</header>}
                    {!appbarChildren && props.children}
                    {appbarChildren && <div className='relative flex-grow'>{props.children}</div>}
                </div>
                {props.floatingActionButton && <aside className='absolute bottom-4 right-4 z-10'>{props.floatingActionButton}</aside>}
            </div>
